...
class Trainer:
    def __init__(self, logger, config, distributed=False):
        ...
        if self.distributed:
            ...
            if self.train_config.num_batch_accumulated % self.num_gpus != 0:
                warnings.warn(
                    f"Running train_config.num_batch_accumulated == {self.train_config.num_batch_accumulated} on {self.num_gpus} GPUs, GPUs will get different number of minibatches, which is inefficient. Consider changing num_batch_accumulated and batch_size.",
                    RuntimeWarning)
        else:
            self.local_rank = 0
            self.num_gpus = 1

        # Each GPU has its own seed for data generation Make extra seed a multiple of 42 (this has to help!) so that
        # runs with successive outer seeds have different seeds here
        ...
        with self.init_random:
            ...
            if self.distributed:
                # this code helps to avoid deadlock/conflicts happening when at model construction there is a need to
                # precompute some files (e.g., preprocess Glove embeddings) we do this first on the main thread and
                # only afterwards on the others
                torch.distributed.barrier()
                if self.local_rank > 0:
                    torch.distributed.barrier()

            self.model_preproc = registry.instantiate(registry.lookup('model', config['model']).Preproc,
                                                      config['model'],
                                                      unused_keys=('name',))
    ...
    def train(self, config, modeldir, tb_name):
        ...
        with self.init_random:
            # We may be able to move optimizer and lr_scheduler to __init__ instead. Empirically it works fine. I
            # think that's because saver.restore resets the state by calling optimizer.load_state_dict. But,
            # if there is no saved file yet, I think this is not true, so might need to reset the optimizer manually?
            # For now, just creating it from scratch each time is safer and appears to be the same speed,
            # but also means you have to pass in the config to train which is kind of ugly.
        ...
        # _path = 'logdir/grappa_qdmr_train/bs=6,lr=7.4e-04,bert_lr=3.0e-06,end_lr=0e0,att=1'
        # _step = 5000
        # pretrain_step = saver.restore(_path, step=_step, map_location=map_location, item_keys=["model"])
        # # saver.save(modeldir, pretrain_step)  # for evaluating pretrained models
        # last_step = pretrain_step
        ...
        with self.data_random:
            ...
            train_dataloader_params = {"batch_size": self.train_config.batch_size,
                                       "num_workers": config[
                                           "num_dataloading_workers"] if "num_dataloading_workers" in config else 0,
                                       # by default load data in the main thread
                                       "shuffle": True,
                                       "drop_last": True,
...
def main(args, distributed=False):
    if args.config_args:
        ...
        config = json.loads(_jsonnet.evaluate_file(args.config))  # base.jsonnet

    if 'model_name' in config:
        args.logdir = os.path.join(args.logdir, config[
            'model_name'])  # logdir/bert_qdmr_train/bs=6,lr=7.4e-04,bert_lr=3.0e-06,end_lr=0e0,att=1

    # Initialize the logger
    reopen_to_flush = config.get('log', {}).get('reopen_to_flush')
    logger = Logger(os.path.join(args.logdir, 'log.txt'),
                    reopen_to_flush)  # bs=6,lr=7.4e-04,bert_lr=3.0e-06,end_lr=0e0,att=1/log.txt
