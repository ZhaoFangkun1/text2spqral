...
def relative_attention_logits(query, key, relation):
    ...
    # query: [batch, heads, num queries, depth]. 1 * 8 * 29 * 128
    # key: [batch, heads, num kvs, depth].  1 * 8 * 29 *128
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    p_attn = F.softmax(scores, dim=-1)  # 注意力得分
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        # 判断h是否能被d_model整除 之后要给每个头分配等量的词特征.也就是embedding_dim/head个.
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)  # 将embedding 拆分成head*d_k
...
class MultiHeadedAttentionWithRelations(nn.Module):
    ...
    def forward(self, query, key, value, relation_k, relation_v, mask=None):
        # query shape: [batch, num queries(ques + tbl + col + val), d_model]  1*29*1024
        # key shape: [batch, num kv(和num queries相同), d_model]
        # value shape: [batch, num kv(和num queries相同), d_model]
        ...
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]  # 1*8*29*1024
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer, layer_size, N, tie_layers=False):  # transformer.EncoderLayer 1024 8
    ...
    def forward(self, x, relation, mask):
        ...
        for layer in self.layers:  # 每一层EncoderLayer都算
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, size, self_attn, feed_forward, num_relation_kinds, dropout):
        ...
        self.self_attn = self_attn  # MultiHeadedAttentionWithRelations
        self.feed_forward = feed_forward  # PositionwiseFeedForward
        self.sublayer = clones(lambda: SublayerConnection(size, dropout), 2)
        self.size = size  # 1024

        self.relation_k_emb = nn.Embedding(num_relation_kinds, self.self_attn.d_k)  # 63 1024
