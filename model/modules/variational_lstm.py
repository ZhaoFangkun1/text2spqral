...
class RecurrentDropoutLSTMCell(torch.jit.ScriptModule):
    ...
    def __init__(self, input_size, hidden_size, dropout=0.):
        ...
        # 输入门
        self.W_i = torch.nn.Parameter(torch.empty(hidden_size, input_size))  # 512 1600
        self.U_i = torch.nn.Parameter(torch.empty(hidden_size, hidden_size))  # 512 512
        # 遗忘门
        self.W_f = torch.nn.Parameter(torch.empty(hidden_size, input_size))  # 512 1600
        self.U_f = torch.nn.Parameter(torch.empty(hidden_size, hidden_size))  # 512 512
        # cell
        self.W_c = torch.nn.Parameter(torch.empty(hidden_size, input_size))  # 512 1600
        self.U_c = torch.nn.Parameter(torch.empty(hidden_size, hidden_size))  # 512 512
        # 输出门
        self.W_o = torch.nn.Parameter(torch.empty(hidden_size, input_size))  # 512 1600
        self.U_o = torch.nn.Parameter(torch.empty(hidden_size, hidden_size))  # 512 512

        self.bias_ih = torch.nn.Parameter(torch.empty(4 * hidden_size))  # 4*512
        self.bias_hh = torch.nn.Parameter(torch.empty(4 * hidden_size))  # 4*512
        ...
        if version.parse(torch.__version__) < version.parse("1.9"):
            # These two lines seem to be some legacy, which we don't quite understand, experimentally those are not needed in torch 1.9
    ...
    def forward(
        ...
        xi_t = torch.nn.functional.linear(input * self._input_dropout_mask[0, :input.shape[0]],
                                          self.W_i)  # 512->1600   #self._input_dropout_mask[0, :input.shape[0]] = 0.7931
        xf_t = torch.nn.functional.linear(input * self._input_dropout_mask[1, :input.shape[0]], self.W_f)  # 512->1600
        xc_t = torch.nn.functional.linear(input * self._input_dropout_mask[2, :input.shape[0]], self.W_c)  # 512 1856
        xo_t = torch.nn.functional.linear(input * self._input_dropout_mask[3, :input.shape[0]], self.W_o)  # 512 1856

        hi_t = torch.nn.functional.linear(h_tm1 * self._h_dropout_mask[0, :input.shape[0]], self.U_i)  # 512 512
        hf_t = torch.nn.functional.linear(h_tm1 * self._h_dropout_mask[1, :input.shape[0]], self.U_f)  # 512 512
        hc_t = torch.nn.functional.linear(h_tm1 * self._h_dropout_mask[2, :input.shape[0]], self.U_c)  # 512 512
        ho_t = torch.nn.functional.linear(h_tm1 * self._h_dropout_mask[3, :input.shape[0]], self.U_o)  # 512 512
