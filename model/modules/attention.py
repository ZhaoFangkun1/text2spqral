...
class ScaledDotProductPointer(torch.nn.Module):
    def __init__(self, query_size, key_size):  # 512 768
        super().__init__()
        self.query_proj = torch.nn.Linear(query_size, key_size)  # 将query映射到key一样大小
    ...
    def forward(self, query, keys, attn_mask=None):
        ...
        attn_logits = torch.bmm(keys, proj_query).squeeze(2) / self.temp  # 1*29
...
class BahdanauPointer(torch.nn.Module):
    ...
    def forward(self, query: torch.Tensor, keys: torch.Tensor, attn_mask=None):
        ...
        query_expanded = query.unsqueeze(1).expand(-1, keys.shape[1], -1)  # 1*29*512 1*29*768
        # scores shape: batch x num keys x 1
        attn_logits = self.compute_scores(
            # shape: batch x num keys x query_size + key_size   # 1*29*1280
