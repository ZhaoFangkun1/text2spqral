...
class TreeTraversal:
    ...
    def step(self, last_choice, extra_choice_info=None):
        ...
        if self.last_handler_name == 'process_sum_inquire':
            self.count_steps += (last_choice in all_next_steps)
    ...
    def process_sum_inquire(self, last_choice):
        # 1. ApplyRule, like step->NextStep******
        # a. Ask which one to choose
        self.save_step_to_log(self.cur_item.node_type, self.cur_item.parent_h_idx, self.prev_action_emb_type,
                              self.prev_action_emb_idx)
    ...
    def process_sum_apply(self, last_choice):
        ...
        sum_type, singular_type = self.preproc.all_rules[last_choice]  # ('step', 'NextStepSelect'): 34
    ...
    def process_children_inquire(self, last_choice):
        # 2. ApplyRule, like NextStepSelect->select
        # Check if we have no children
        # a. Ask about presence
        type_info = self.preproc.ast_wrapper.singular_types[self.cur_item.node_type]
    ...
    def process_children_apply(self, last_choice):
        ...
        for field_info, present in reversed(
            ...
            elif field_type in self.preproc.ast_wrapper.sum_types:  # step
                child_state = TreeTraversal.State.SUM_TYPE_INQUIRE
            elif field_type in self.preproc.ast_wrapper.product_types:  # select comparative
    ...
    def process_ref_inquire(self, last_choice):
        ...
        self.save_step_to_log(self.cur_item.node_type, self.cur_item.parent_h_idx, self.prev_action_emb_type,
                              self.prev_action_emb_idx, count_steps=self.count_steps)
        self.cur_item = attr.evolve(self.cur_item,
        ...
        choices = self.ref_choice(self.cur_item.node_type, step_history=self.cur_item.step_history,
                                  grounding=self.cur_item.grounding)
        return choices, False
