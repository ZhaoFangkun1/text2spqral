...
class BreakDecoder(torch.nn.Module):
    ...
    def __init__(
            ...
            rule_emb_size=128,
            node_embed_size=64,
            enc_recurrent_size=256,  # 1024 or 768
            recurrent_size=256,
            dropout=0.,
            desc_attn='bahdanau',
            copy_pointer=None,
        ...
        self.node_type_vocab = vocab.Vocab(
            sorted(self.preproc.primitive_types) +  # int str......
            sorted(self.ast_wrapper.custom_primitive_types) +  # ref、grounding  2个
            sorted(self.preproc.sum_type_constructors.keys()) +  # 14个
            sorted(self.preproc.field_presence_infos.keys()) +  # 由sum的产生式右端是否还存在继续 54
        ...
        self.rule_embedding = torch.nn.Embedding(  # len(self.rules_index)-> rule_emb_size 128
        ...
        self.pointer = attention.ScaledDotProductPointer(
            query_size=self.recurrent_size, # 512
            key_size=self.enc_recurrent_size) # 768
        ...
        self.ref_embedding = torch.nn.Embedding(  # 20->128
    ...
    def compute_loss_batched(self, enc_inputs, examples, desc_encs, execution_maps, debug):
        ...
        mle_losses = self.compute_loss_given_execution_plan_batched(execution_maps, desc_encs)  # 生成(标答) 和 enc_states

        assert not self.use_align_loss
        return mle_losses

    @staticmethod
    def compute_decoder_input(enc_input, example, desc_enc,  # enc_input 标答  encState
                              exclude_rules_loss,
                              preproc,
                              sup_att,
                              attn_type,
                              debug=False):
        grnd_idx = desc_enc.grnd_idx  # 0
        debug_for_excluding = len(exclude_rules_loss) != 0
        rules_index = {v: idx for idx, v in enumerate(preproc.all_rules)}

        traversal = TrainTreeTraversal(preproc, desc_enc, rules_index, exclude_rules_loss=exclude_rules_loss,
                                       debug=debug_for_excluding)

        traversal.step(None)
        queue = [
            TreeState(
                node=example.tree[grnd_idx],  # 来自dec_train的标答
                parent_field_type=preproc.grammar.root_type,  # 'root'
            )
        ]
        while queue:
            item = queue.pop()
            ...
            if parent_field_type in preproc.grammar.ref:  # ref
                assert isinstance(node, int)
                assert traversal.cur_item.state == TreeTraversal.State.REF_APPLY
                traversal.step(node)
                continue

            if parent_field_type in preproc.grammar.general_pointers:  # grounding
                assert isinstance(node, int)
            ...
            if parent_field_type in preproc.ast_wrapper.primitive_types:  # 'string', 'object', 'grounding', 'bytes', 'int', 'ref', 'identifier', 'singleton'
            ...
            # 'root': Product([Field(step, step)]) #下一步骤的信息
            # 'NextStepSelect':'NextStepSelect': Constructor(NextStepSelect, [Field(select, select)]),
            # 'select': Product([Field(is_distinct, is_distinct), Field(grounding, grounding), Field(step, step)]),
            # 'NextStepComp': Constructor(NextStepComp, [Field(comparative, comparative)]),
            # 'comparative': Product([Field(is_distinct, is_distinct), Field(ref, ref1), Field(ref, ref2),
            #                        Field(comparative_3arg_type, comparative_3arg_type), Field(step, step)]),

            if parent_field_type in preproc.sum_type_constructors:  # agg_ops、agg_type、column_type、comp_op_type、comp_val、comparative_3arg_type、comparative_ops、is_distinct、order、project_1arg、step、superlative_op_type、superlative_ops、union_3arg
            ...
            if type_info.fields:
                ...
                rule_idx = rules_index[rule]  # ('NextStepSelect', (True,)): 23
    ...
    def compute_loss_given_execution_plan_batched(self, decoder_inputs_batch, desc_enc_batch):  # 标答  来自enc的输入
        model = self
        ...
        desc_enc_batch = [desc_enc_batch[i_] for i_ in new_batch_order]  # 按照标答的长度重新排列
        ...
        encoder_memory = torch.nn.utils.rnn.pad_sequence([desc_enc.memory.squeeze(0) for desc_enc in desc_enc_batch],
                                                         batch_first=True)
        ...
        for i_b in range(batch_size):
            m2c_align_mat_batch[i_b, :desc_enc_batch[i_b].m2c_align_mat.size(0),
            :desc_enc_batch[i_b].m2c_align_mat.size(1)] = \
                desc_enc_batch[i_b].m2c_align_mat

        # prepare node embeddings
        # node_type_emb shape: batch (=1) x 64
        ...
        for i_b in range(batch_size):
            ...
            for i_step in range(len(decoder_inputs_batch[i_b]) - 1):  # the last item is a barrier
                ...
                node_type_idx = model._index(model.node_type_vocab, node_type)
        ...
        for i_b in range(batch_size):
            ...
            for i_step in range(len(decoder_inputs_batch[i_b]) - 1):  # the last item is a barrier #标答
                step_input = decoder_inputs_batch[i_b][i_step]
                node_type = step_input["node_type"]
                step_input_next = decoder_inputs_batch[i_b][
                    i_step + 1]  # different step index here is an artifact of information collection - this is ugly indeed
                ...
                if action_emb_type == "zero_rule_emb":
                    ...
                    pointer_action_emb_proj = model.pointer_action_emb_proj
        ...
        for i_b in range(batch_size):
            action_emb_idx_for_step = [action_embeddings_index_for_key[key] for key in
                                       action_emb_keys_for_batch_and_step[i_b]]
        ...
        # prepare indices for parent actions   parent_node_idx
        parent_node_idx_for_step_and_batch = []
        for i_b in range(batch_size):
            parent_node_idx_for_step = []  # 一个数据的所有step
            for i_step in range(len(decoder_inputs_batch[i_b]) - 1):  # the last item is a barrier
        ...
        action_embs = model.zero_rule_emb.expand(batch_size, -1).unsqueeze(
            0)  # clone for the batch and add dummy dimension for the sequence length
        ...
        for i_step in range(max_num_steps):  # 按长度处理 先处理step较少的
            ...
            desc_context, attention_logits = model._desc_attention_batched(recurrent_state_cur,
                                                                           encoder_memory_cur,
                                                                           encoder_memory_mask_cur.unsqueeze(
                                                                               1))  # prepare the dimensions for multi-headed attention
            # get node embeddings
            node_type_idx_step = node_type_idx_for_step_and_batch_cur[i_step]
            node_type_emb = model.node_type_embedding(
                node_type_idx_step)  # for torch.cat(dim=1): batch_size x feature_dim
            ...
            parent_action_idx = torch.gather(action_emb_idx_for_step_and_batch_cur, 0,
                                             parent_node_idx.unsqueeze(0)).squeeze(0)  # batch_size
            parent_action_emb = action_embedding_layer(
                parent_action_idx)  # for torch.cat(dim=1): batch_size x feature_dim

            outputs_cur = outputs[:, :batch_size_cur]
            idx_for_gather = parent_node_idx.unsqueeze(0).unsqueeze(2).expand(1, outputs_cur.size(1), outputs.size(
                2))  # 1 x batch_size x feature_dim
            parent_h = torch.gather(outputs_cur, 0, idx_for_gather).squeeze(
                0)  # for torch.cat(dim=1): batch_size x feature_dim

            # prepare input to the decoder cell
            state_input = torch.cat(
                (
                    action_emb,  # a_{t-1}: rule_emb_size 128
                    desc_context,  # c_t: enc_recurrent_size 768
                    parent_h,  # s_{p_t}: recurrent_size 512
                    parent_action_emb,  # a_{p_t}: rule_emb_size 128
                    node_type_emb,  # n_{f-t}: node_emb_size 64
                ),
                dim=-1)

            # update decoder state
            recurrent_state_cur = model.state_update(
                # state_input shape: batch (=1) x 1600
                state_input, recurrent_state_cur)

            output = recurrent_state_cur[0] # 即将用于计算得分
            if output.size(0) < batch_size:
                # pad output vector with zeros
                output = torch.cat([output,
                                    torch.zeros((batch_size - output.size(0), output.size(1)), device=output.device,
                                                dtype=output.dtype)],
                                   dim=0)
            outputs = torch.cat([outputs, output.unsqueeze(0)],
                                dim=0)  # add current output to the tensor of all outputs
        ...
        if loss_mask.sum().item() > 0:  # have any elements with this loss
            ...
            good_node_types = [model.node_type_vocab.elem_to_id[key] for key in ["grounding", "column"] if
                               key in model.node_type_vocab.elem_to_id]
            assert len(good_node_types) == 1, f"Expecting only nodes of node_type 'grounding' for the pointer loss"
            assert (node_types == good_node_types[
                0]).all().item(), f"Expecting only nodes of node_type 'grounding' for the pointer loss"
            ...
            attn_mask_for_loss = ~torch.index_select(encoder_memory_mask, 0,
                                                     batch_index_for_loss)  # need inverted mask here
        ...
        if loss_mask.sum().item() > 0:  # have any elements with this loss
            ...
            assert (
                    ref_count_steps_for_loss != ref_count_step_barrier).all(), f"Value {ref_count_step_barrier} should not appear in the ref loss"

            ref_logits_mask_inf = torch.arange(ref_logits.size(1), device=ref_logits.device)[None, :] >= (
                    ref_count_steps_for_loss[:, None] - 1)
    ...
    def _update_state(
            self,
            node_type,
            prev_state,  # 上一时刻的h_t和c_t
            prev_action_emb, prev_action_emb_type, prev_action_emb_idx,
            parent_h, parent_h_idx,
            parent_action_emb,
            desc_enc):
        # desc_context shape: batch (=1) x emb_size
        desc_context, attention_logits = self._desc_attention(prev_state, desc_enc)  # 上下文向量
        ...
        state_input = torch.cat(
            (
                prev_action_emb,  # action_{t-1}: rule_emb_size # 128
                desc_context,  # context_t: enc_recurrent_size   # 768
                parent_h,  # state_{p_t}: recurrent_size       # 512
                parent_action_emb,  # action_{p_t}: rule_emb_size # 128
                node_type_emb,  # node_type_{f-t}: node_emb_size     # 64
            ),
            dim=-1)
    ...
    def apply_rule(
        ...
        new_state, _ = self._update_state(
            ...
            parent_action_emb,
            desc_enc)
        # output shape: batch (=1) x 512
        output = new_state[0]
        # rule_logits shape: batch (=1) x num choices
        rule_logits = self.rule_logits(output)  # 各选择得分
        ...
        if not is_train and step_history is not None:
            if node_type in ['comp_op_type', 'column_type', 'superlative_op_type']:
                ...
                if is_comparative:
                    ...
                    if self.val_types_wo_cols:
                        ...
                        # if filter and no_column - should be NoOp (and NoColGrnd because of no_column) -> tbls,
                        # cols if filter and not no_column - should be ColGrnd if CompOp -> tbls, cols if NoOp or
                        # val+col if CompOp  or NoOp and ColGrnd if comp and no_column - can be NoOp/CompOp (and
                        # NoColGrnd because of no_column) -> ref if comp and not no_column - if CompOp, ColGrnd ->
                        # val + col - if NoOp, ColGrnd -> val + col - if NoOp/CompOp, NoColGrnd -> ref
                ...
                check_no_values = lambda end_node: self.no_vals \
                                                   and (
                                                           end_node == 'ColumnGrounding' or is_filter and end_node == 'CompOp')
                ...
                for (start_node, end_node), idx in self.rules_index.items():
                    ...
                    if check_unknown(end_node) or check_no_values(end_node) or \
                            check_required_column(end_node) or check_no_column(end_node) or check_col_type_match(
                        end_node):
                        continue
            ...
            if node_type == 'comp_val':
                ...
                if not is_filter and (self.no_vals or not col_grnd and (self.required_column \
                                                                        or self.val_types_wo_cols and not self.val_types_wo_cols.intersection(
                            lhs_val_type))):
                    rules_start += 1
    ...
    def compute_pointer(
        ...
        pointer = self.pointer  # 注意力
        pointer_logits = pointer(
            output, desc_enc.pointer_memories[node_type])  # grounding部分的表示
