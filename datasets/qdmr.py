...
def create_qdmr_ast(qdmr_entry, grounding):
    ...
    for i, (op, all_args) in enumerate(zip(qdmr_entry.ops, qdmr_entry.args)):
        ...
        assert op != 'arithmetic' or op == 'arithmetic' and len(all_args) <= 3, 'Arithmetic with {} args'.format(
            len(all_args))
        ...
        for i_arg, arg in enumerate(all_args):
            ...
            if refs and cur_grounding:
                assert op in ('comparative', 'filter') and cur_grounding.iscomp(), \
                    'ref and grounding can be only in comparative with ref arg, got {} op and {}, {}'.format(op, refs,
                                                                                                             cur_grounding)
                ...
                assert op == 'project' and i_arg == 0 or op == 'comparative' and i_arg == 2 \
                       or op == 'filter' and i_arg == 1, 'None arg can be in project and comparative, got {}, index of arg: {}'.format(
                    op, i_arg)
...
class BreakDataset(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        subset_name = break_data.get_dataset_keyword_from_name(break_data.names[0])  # SPIDER
        assert subset_name == 'SPIDER'
        break_partition = self.partition if self.partition != 'test' else 'dev'

        print('Loading, {} part'.format(self.partition))

        for subset_idx, spider_entry in enumerate(tqdm(spider_data)):  # for all spider
            db_id = spider_entry['db_id']
            name = break_data.get_item_name(subset_name, break_partition, subset_idx)  # SPIDER_train_idx

            if name in break_data.names:  # with QDMR
                i = break_data.names.index(name)
                question = break_data.qdmr_full_table[i][1]  # 相当于train_spider.csv的question_text
                if spider_entry['question'] != question:  # 如果不一致按spider为准
                ...
                continue
            qdmr_entry = break_data.qdmrs.get(name)  # 1: SELECT['singers'] 2: AGGREGATE['count',  '#1']
            groundings = self.grounding_list.get(name)  # grounding信息
