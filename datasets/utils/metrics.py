...
class Metrics:
    ...
    def postprocess_inferred_code(self, inferred_code):
        ...
        qdmr_op_args = []
        grounding = dict()
        # [["select", [{"arg_type": "grounding", "arg": {"type": "tbl", "keys": ["singer"]}}], false],
        #  ["aggregate", [{"arg_type": "grounding", "arg": "count"}, {"arg_type": "ref", "arg": ["#1"]}]]],
        for i, qdmr_step in enumerate(inferred_code):
            ...
            for i_arg, step_arg in enumerate(args_list):
                # [{"arg_type": "grounding", "arg": {"type": "tbl", "keys": ["singer"]}}]
    ...
    def compare_sql_qdmr(self, qdmr, sql, schema, rdf_graph, grnd, tl=60, ordered=False, verbose=True):
        ...
        try:
            with time_limit(tl):
                ...
                try:
                    result = QueryResult.execute_query_to_rdf(sparql_query, rdf_graph, schema,
                                                              virtuoso_server=self.virtuoso_server)
        ...
        return got_correct_answer, error_details, sql_result, sparql_query, result
    ...
    def _evaluate_one(self, break_item, inferred_code, section, check_gold=False, verbose=False):
        ...
        if break_item.schema:
            ...
            got_correct_answer, error_details, sql_result, sparql_query, result = self.compare_sql_qdmr(
                qdmr, sql, schema, rdf_graph, grounding, ordered=ordered, verbose=verbose)

        if section != 'test':
            gold_qdmr = QdmrInstance(break_item.qdmr_ops, break_item.qdmr_args)
            gold_grounding = break_item.grounding[0]
            if check_gold:
                gold_got_correct_answer, error_details, sql_result, sparql_query, result = self.compare_sql_qdmr(
                    gold_qdmr, sql, schema, rdf_graph, gold_grounding, ordered=ordered, verbose=verbose)
        ...
        if verbose and error_details and error_details['sparql_error_type'] in (
                'SparqlGenerationError, SparqlRunError'):
            print()
        ...
        return exact_match, got_correct_answer, sql_hardness, qdmr, grounding, sql_result, sparql_query, result

    def add(self, break_item, inferred_code, section):
        if not break_item:
            return

        exact_match, exec_match, sql_hardness, qdmr, grounding, sql_result, sparql_query, result = self._evaluate_one(
            break_item,
            inferred_code,
            section)
        self.scores[section][break_item.full_name] = {'ex': exec_match, 'em': exact_match, 'hardness': sql_hardness}
        return exact_match, exec_match, sql_hardness, break_item.orig_spider_entry[
            'question'], qdmr, grounding, break_item.db_id, break_item.orig_spider_entry[
                   'query'], sql_result, sparql_query, result
    ...
    def finalize(self):
        ...
        res = {
            'per_item': [{'name': name, 'ex': score['ex'], 'em': score['em'], 'hardness': score['hardness']} for sec_d
                         in self.scores.values() for name, score in sec_d.items()],
            'total_scores': {'ex': mean_exec_match, 'em': mean_exact_match},
        }

        levels = {'easy', 'medium', 'hard', 'extra'}
        for section in self.scores.keys():
            res['total_scores']['ex_{}'.format(section)] = float(
                np.mean([el['ex'] for el in self.scores[section].values()]))
            for lvl in levels:
                res['total_scores']['ex_{}_{}'.format(section, lvl)] = float(
                    np.mean([el['ex'] for el in self.scores[section].values() if el['hardness'] == lvl]))
