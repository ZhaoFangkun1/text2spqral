...
class ValueExtractor:
    def __init__(self, schemas, extract_value, partition):
        ...
        self.db_table_column_info = {}  # 每个数据库的表、列信息 key为数据库id
        self.db_data = {}
    ...
    def get_values(self, qdmr_entry, grounding, db_id, question, break_idx):
        ...
        if db_id is not None and self.db_table_column_info.get(db_id) is None:
            ...
            self.sqlite_types(db_id) #将每个表每列的类型归为text number time others
    ...
    @staticmethod
    def extract_numbers_dates_from_text(tokens):  # 从问题中提取
        val_unit_list = []
        ...
        for i in range(len(tokens)):
            for j in range(i + 1, len(tokens)):
                ...
                try:
                    ...
                    val_unit_list.append(
                        ValueUnit(d, value_type="time", orig_value=s, q_match=q_val_match, source="text"))
                except:
                    pass
    ...
    def sqlite_types(self, db_id):
        ...
        for table in self.schemas[db_id].tables:  # 对于每一个表
            table_name = table.orig_name
            self.db_table_column_info[db_id][table_name] = {}

            cur = conn.execute("PRAGMA table_info('{}') ".format(table_name))
            for j, col in enumerate(cur.fetchall()):
                column_name = col[1]  # 列名

                # varchar, '' -> text, int, numeric -> integer, ...
                col_type = col[2].lower()
                if 'char' in col_type or col_type == '' or 'text' in col_type or 'var' in col_type:  # char text var归为text
                    col_type = 'text'
            ...
            for val_unit in column_data:
                ...
                if is_number:
                    ...
                    if word in value:
                        ...
                        if (val_unit,1.0) not in column_data_matches:
                            column_data_matches.append((val_unit, 1.0))
                    elif ps.stem(value) == ps.stem(word) or sno.stem(value) == sno.stem(word):
                        value = transform_to_type(val_unit.orig_value, value_type)
                        val_unit = attr.evolve(val_unit, value=value, value_type=value_type, q_match=q_val_match)
                        if (val_unit,1.0) not in column_data_matches:
                            column_data_matches.append((val_unit, 1.0))
                    elif similarity >= threshold and not is_number:
                        value = transform_to_type(val_unit.orig_value, value_type)
                        val_unit = attr.evolve(val_unit, value=value, value_type=value_type, q_match=q_val_match)
    ...
    def values_in_grnd(self, grounding_list, db_id):
        ...
        if grounding.isval():
            ...
            if op in ('max', 'min') or not orig_value or orig_value.find('#') != -1:  # 并到sup
                return
    ...
    def get_values_from_qdmr(self, qdmr_entry, grounding, question_tokens, db_id, verbose=False):
        ...
        for i, all_args in enumerate(
                qdmr_entry.args):  # [['heads of departments'], ['#1', 'older than 56'], ['count', '#2']]

            for i_arg, arg in enumerate(all_args):  # ['heads of departments']
                grnd_index = GroundingIndex(i, i_arg, arg)  # 0|0|heads of departments
                cur_grounding_list = [grounding.get(grnd_index)]  # [tbl | head]
                ...
                if cur_grounding_list[0] and cur_value:
                    if self.matching:
                        arg_tokens = self.tokenize(arg)  # qdmr的参数token
                        matches = []
                        for arg_tok in arg_tokens:
                            for i, tok in enumerate(question_tokens):  # question token和qdmr arg token有匹配项
                                if ps.stem(arg_tok) == ps.stem(tok) or sno.stem(arg_tok) == sno.stem(tok):
                                    matches.append(i)  # 添加question token的id
                        ...
                        if not matches:
                            ...
                            if tokenized_arg in tokenized_question:  # 完全匹配
                                match_type = 'VEM'
                            else:
                                match_type = 'VPM'
    ...
    def tokenize(self, text):
        ann = corenlp1.annotate(text, ['tokenize', 'ssplit'])
    ...
    def convert_quotes(self, question, question_tokens):
        ...
        for quote in quote_symbols:  # 反斜杠
        ...
        for tok in tokens_in_quotes:
            ...
            for res, res_type in zip(results, res_types):
                converted_toks.append(
                    ValueUnit(res, value_type=res_type, orig_value=tok, q_match=q_val_match, source="text"))
