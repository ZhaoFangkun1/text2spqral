...
@attr.s
class Schema:  # 定义schema
    db_id = attr.ib()
    tables = attr.ib()
    columns = attr.ib()
    foreign_key_graph = attr.ib()
    orig = attr.ib()
    database_schema = attr.ib()
    connection = attr.ib(default=None)


def postprocess_original_name(s: str):
    return re.sub(r'([A-Z]+)', r' \1', s).replace('_', ' ').lower().strip() # 大写->小写 ' '替换'_'
...
def load_tables(paths):
    ...
    for path in paths:
        ...
        for schema_dict in schema_dicts:
            ...
            schemas[db_id] = Schema(db_id, tables, columns, foreign_key_graph, schema_dict, database_schema)
