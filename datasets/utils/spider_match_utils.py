...
def compute_schema_linking(question, column, table, value=None):
    def partial_match(x_list, y_list):  # 局部匹配
        x_str = " ".join(x_list)
        y_str = " ".join(y_list)
    ...
    def exact_match(x_list, y_list):  # 完全匹配
        x_str = " ".join(x_list)
        y_str = " ".join(y_list)
        # print(x_str,y_str)
        if x_str == y_str:
    ...
    while n > 0:
        for i in range(len(question) - n + 1):
            ...
            for col_id in col_id2list:
                if exact_match(n_gram_list, col_id2list[col_id][:-1]):
                    # print('exact match with ',col_id2list[col_id][:-1])
